            ...
            <TouchableWithoutFeedback onPress={() => props.setApartModalVisible(false)} style={{zIndex: 1}}>
              <ModalBackground style={{zIndex: 1}}>
                <ModalComponent>
                  {Apartlist.map((apart) => (
                    <View key={apart.id}>
                      <ModalSubComponent onPress={() => {props.setSelectedApart(apart.name); props.setApartModalVisible(false);}}>
                        <MaterialIcons name='apartment' size={25}></MaterialIcons>
                        <ModalText> {apart.name} {apart.default ? "(기본)" : ""}</ModalText>
                      </ModalSubComponent>
                      <DistinctLine/>
